...
function sqlCheckSchedulePlanExit($idSchedule,$startDate,$startTime)
{
    ...
    {      
        $id = NULL;
        $SQLSelect = <<<SQL
            SELECT schedule_id FROM tbplainscheduleset WHERE schedule_id=:schedule_id AND active_start_date=:active_start_date AND active_start_time=:active_start_time;
SQL;
        $sth = ConnectDB::$dbh->prepare($SQLSelect);                       

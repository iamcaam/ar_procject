...
class JobAction extends BaseAPI
{
	...
	function createSnapshotJob($input,&$output)
	{
	    ...
	    foreach ($input['VDs'] as $value) {
	    	$output_check = null;
	    	$vd_c->sqlListVDUserIDByVDID($value,$output_check);	    	
	...
	function insertCreateSnapshotJobLog($code,$vdName,$domainName,$domainID,$appendErr)
    {
        $logType = 9;
        $logLevel = 3;
        ...
        if(is_null($appendErr)){
            $message = "Create snapshot job$appendVDDomain Success";
        }
        else{
            $message = "Failed to create snapshot job$appendVDDomain$appendErr";
        }
    ...
    function insertCreateBackupJobLog($code,$vdName,$domainName,$domainID,$appendErr)
    {
    	$logType = 9;
        $logLevel = 3;
        if(is_null($appendErr)){
        ...
        $appendVDDomain = $this->logappendVDDomain($vdName,$domainName);   
        if(is_null($appendErr)){
            $message = "Create backup job$appendVDDomain Success";
        }
        else{
            $message = "Failed to create backup job$appendVDDomain$appendErr";
        }
        if(LogAction::createLog(array('Type'=>$logType,'Level'=>$logLevel,'Code'=>$code,'Riser'=>'admin','Message'=>$message),$lastID)){
            LogAction::sqlInsertDomainLogRelation($lastID,$domainID);
        }       

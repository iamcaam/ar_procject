...
var RestoreEXIPList = []; // 新增外部IP及轉址功能設定  2017.09.05 william
var EXIPListCount = []; // 新增外部IP及轉址功能設定  2017.09.06 william
var oUIAD = {}; // 新增AD功能設定  2017.09.06 william
var oUIUPS = {}; 
var oUIEmail = {};

var IDE_type_count; // 2018.01.08 william 新增磁碟擴充功能
var SCSI_type_count; // 2018.01.08 william 新增磁碟擴充功能
var VirtIO_type_count; // 2018.01.08 william 新增磁碟擴充功能
var SysCfg_lang_ver; // 2018.01.08 william 列表標題及內容的文字位置調整

var UserNameArray;
var SeedNameArray;
var VMUserListflag;
var VDUserListflag = false;
var ImportUserListflag = false;
var SeedUserListflag = false;
var temp_username_for_import;
var VMListUserName = [];
var BornedListUserName = [];
var TextAreaUserName = [];
...
var AjaxData_for_Import;
var browserNeedChanage;
var nowUserWidth;
var JobsID;
function BrowserDetect() {
    browserNeedChanage = false;
    if(!$.browser){
        $.browser = {};
        ...
        if(browserNeedChanage) {            
            $('#listview_content').css("overflow-y","auto"); 
            $('#Dialog_listview_content').css("overflow-y","auto");
            $('#Dialog_Seedlistview_content').css("overflow-y","auto");            
            $('#listview_content_cb_clone').css("overflow-y","auto");
            $('#listview_content_cb_bornuser').css("overflow-y","auto");
        }
    }      
}
...
function formatGB(GB, decimals) {
    ...
    return add_comma_of_number(parseFloat((GB / Math.pow(k, i)).toFixed(dm))) + ' ' + sizes[i];
}

function DataFormatSize(value) {  
...
function InitialSettingAndEvent() {
    ...
    InitUserMgt();
    /*--------------------------處理按鈕變色問題 2017.04.21 william--------------------------*/
    $('.btn-jquery').on( "mouseenter", function() {
    ...
    CreateDialog_ExpendDisk();
    // 2018.01.08 william 列表標題及內容的文字位置調整
    ChecklangVer();
...
function RegAllPageEvent() {
    ...
    oUIAD = UIADControl.createNew(oHtmlCreate, oRelayAjax); // 新增AD功能設定  2017.09.06 william
    oUIUPS = UISysUPS.createNew(oHtmlCreate, oRelayAjax);
    oUIEmail = UISysEmail.createNew(oHtmlCreate, oRelayAjax);
    oUISystemHostName = UISysHostName.createNew(oHtmlCreate, oRelayAjax);
    oUISystem.Init();
    oUIIP.Init();
    oUIAD.Init(); // 新增AD功能設定  2017.09.06 william
    oUIUPS.Init();
    oUIEmail.Init();
    ...
    $('#H5_SubTitle_SysMgr_Network').click(function (event) {
        ...
        oUISystemHostName.ListHostName();
        oUIAD.GetADList();  // 新增AD功能設定  2017.09.06 william
        oUIUPS.ListUPS();
        oUIEmail.ListEmail();
    });

    // User managerment
    $('#H5_SubTitle_SysMgr_AccountMgr_User').click(function (event) {
        var btn_element = $(this);
        MenuButtonClickChangeColor(btn_element, true);   /* 上方大型選單按鈕變色 2017.01.18 william */
        event.preventDefault();
        ChangeDivToLayer($('#Div_SysCfg_UserManagerment'));
        StopPageInterval();

        GetUserDataForCreateTableList();        
        $('#tab-usermanagerment-container').easytabs('select', '#tabs1-user_list');
    });
...
function AddDivToLayer() {
    ...
    $('#Div_Main_LayoutContain').prepend($('#Div_SysCfg_UserManagerment'));
    ChangeDivToLayer($('#Div_SysCfg_Network'));
}
...
function ChangeDivToLayer(obj) {
    ...
    $('#Div_SysCfg_UserManagerment').css("z-index", "0");
...
function RemoveDesignLayoutZone(obj) {
    ...
    $('#Div_LayoutZone_SysCfg_UserManagerment').remove();
}
...
function StopPageInterval() {
    ...
    if (typeof (oUIUPS) !== 'undefinded')
        oUIUPS.ClearUpsInteval();
}
...
$(window).resize(function () {    
    ...
    if(now_page_id == "Div_SysCfg_UserManagerment"){
        OnDomAttrChanged();
    }else{
        stopUserListWidthInterval();
    }
});
...
function reLocateByDiv(id) {
    switch (id) {
        ...
        case 'Div_SysCfg_SysInfo':              // 新增System Infomation layout resize的功能 2017.03.03 william
            ...
            break;
        case 'Div_SysCfg_UserManagerment':         
            relocateUserManagermentLayout();
            break;            
    }
}
...
function relocateSystemsettingLayout() {
    ...
    $('#Div_exipList').height(BrowserHeight - 359); // 新增外部IP及轉址功能設定  2017.09.04 william  外部IP列表
    $('#tabs1-ad').height(DivMainContainHeight - 90); // 新增AD功能設定  2017.09.04 william
    $('#tabs1-ssh').height(DivMainContainHeight - 90);
    $('#tabs1-ups').height(DivMainContainHeight - 90);
    $('#tabs1-email').height(DivMainContainHeight - 90);
}
...
function relocateUserManagermentLayout() {
    var BrowserHeight = $(window).height();
    var BrowserWidth = $(window).width();    
    var DivMainContainHeight = $('.Div_MainContain').height();
    
    $('#tabs1-user_list').height(BrowserHeight - 265);

    var UserListWidth = $('#tabs1-user_list').width();

    $('.listContainer').height(BrowserHeight - 308);
    $('.listContainer').width(UserListWidth * 0.50);
    $('#tab-user-container').height(BrowserHeight - 300);
    $('#tabs1-user_disk').height(BrowserHeight - 361);    
    $('#tabs1-user_Enable').height(BrowserHeight - 361);     
    $('#tabs1-user_password').height(BrowserHeight - 361);    
    $('#listview_content').height(BrowserHeight - 350);     
    $('#tab-user-container').width((UserListWidth * 0.48));
}
...
function CreateDialog_VMCreate() {
    $("#dialog_create_vm").dialog({
        ...
        draggable: true,
        autoOpen: false,
        height: 750, // 新增CPU類型  2018.06.01 william 580 -> 620 -> 700
        width: 750,
        dialogClass: "no-close",
        closeOnEscape: false
    });
...
/* 2018.01.08 william 新增磁碟擴充功能，開啟擴充視窗 */
function CreateDialog_ExpendDisk () {  
    $("#dialog_create_expand_disk").dialog({
        title: LanguageStr_VM.VM_ExpendDisk_title,
        modal: true,
        resizable: false,
        draggable: true,
        closeOnEscape: false,
    ...
    $('#Expanddisk_Btn_Exit').click(function (event) {
        event.preventDefault();
...
// 取得 Raid ID 資訊
function Get_Raid_ID(elementid, initvalue, hidevalueFlag) {          
    var HtmlStr = "";
    var innerStr = "";
    var tempStr = "";
    var idNamedivide = elementid.selector.substring(1);
    ...
    request.done( function( AjaxData, statustext, jqXHR ) {        
        ...
        HtmlStr += '<select id="combo_' + idNamedivide + '">';                           
        $.each( AjaxData, function(index, element) {
            if(element['Status'] != '' || element['Status'].indexOf("creating") !=-1 || element['Status'].indexOf("deleting") !=-1 || element['Status'].indexOf("adding") != -1) { 
                if(!hidevalueFlag)
                    ...
                    if(initvalue == element['RAIDID']) {
                        tempStr  += '<option value="'+ element['RAIDID'] + '">'+ element['RAIDID'] + '</option>';  
                    } else {
                        innerStr += '<option value="'+ element['RAIDID'] + '">'+ element['RAIDID'] + '</option>';  
                    }                        
                }    
        ...
        if(hidevalueFlag && ind < 2) {
            HtmlStr += tempStr;
            $('.move_raid_alert').show();      
        } else if(hidevalueFlag && ind > 1) {
            HtmlStr += innerStr;
        }  
        ...
        if(hidevalueFlag && ind < 2) {
            $('#btn_moveraid_confirm').button( "disable" ); 
            $('#btn_moveraid_forVD_confirm').button( "disable" );             
        } else {
            $('#btn_moveraid_confirm').button( "enable" ); 
            $('#btn_moveraid_forVD_confirm').button( "enable" );                 
        }           
        $('#combo_'+ idNamedivide).scombobox({
            editAble:false,
            wrap:false       
        });         
        // if(initvalue != -1) {
        //     $('#combo_'+ idNamedivide).scombobox('val',initvalue);                
        // }
    });      
...
function Import_Get_Raid_ID() {          
    ...
    function limitText(elementid, maxChar){
        // $(field).attr('maxlength',maxChar);
        elementid.attr('maxlength',maxChar);
    }

...
function GetShareForContent(idR,idO,idU,sharecase,code)
{
    ...
    var request = CallAjax("ShareCmdProcessPost.php?act=ListShareContent", jsonrequest , "json");    
    request.done(function (msg, statustext, jqxhr) {     
//        alert(jqxhr.responseText);
        $.each( msg, function( index, element ) 
        {                           
...
{    
    $("body").css("cursor", "wait");
    openModalWaitNoReload('',0,0);           
    var request = CallAjax_NoAsync("ShareCmdProcessPost.php?act=GetSharePath", postData, "json");    
        request.done(function (msg, statustext, jqxhr) {   
        $.each( msg, function( index, element ) 
        {                               
            switch(index)
...
function OpenShareManageWindow()
{
    window.location.assign('MyShareList.php?sid=' + sid + "&url=" + encodeURIComponent(location.href.replace(/'/g, "%27")));   
//    openModalReget('MyShareList.php?sid=' + sid ,1005,600);
}
...
{
     document.location.hash = '/' + index + '/?act=5&idR=' + ShareFors[index]['idObject'] + '&idO=' + ShareFors[index]['idObject'] + '&idU=' + ShareFors[index]['idUser'] + '&code=' + ShareFors[index]['codeSecurity'] + '&sharecase=' + ShareFors[index]['idShareCase']
}
...
function SANShareFrom(index)
{
    ...
    var postData = '{ "sid":"'+ sid + '","stgid":' + ShareFors[index]['stgid'] +'}';        
    $("body").css("cursor", "wait");
    openModalWaitNoReload('',0,0);            
    ...
    request.done(function (msg, statustext, jqxhr) {               
        ...
        {                
            if(msg['local'] == 1)
            {
                ...
                if(bol_check)
                    redirecturl = 'content_san.php?sid=' + sid + '#/' + index + '/?act=5&idR=' + ShareFors[index]['idObject'] + '&idO=' + ShareFors[index]['idObject'] + '&idU=' + ShareFors[index]['idUser'] + '&code=' + ShareFors[index]['codeSecurity'] + '&sharecase=' + ShareFors[index]['idShareCase'];
            }
            else
            {
                bol_check = true;
...
//            openModal('upload.php?sid='+ sid + '&crt=' + crt + '&cid=' + cid ,400,380);        
    });
}
...
function AddFolderUploadLogoutClick() {
    ...
    {        
        if(Silverlight.isInstalled('5.0')){    
            ...
            {
                case 1:
                    window.location.assign('ARWADD_Upload.php?sid=' + sid + '&idU=-1&crt=' + crt +'&cnt=8&cid=' + cid + '&showdest=' + encodeURIComponent($('#path').text().replace(/'/g, "%27")) + '&sharecase=29');
                    break;
                case 2:
...
function ShareLinkClickFunction() {
    $('.Shared').click(function () {        
        ...
        window.location.assign('ShareMain.php?sid=' + sid + "&id=" + idobj + '&name=' + encodeURIComponent(name.replace(/'/g, "%27")) + '&identify=' + type + "&url=" + encodeURIComponent(location.href.replace(/'/g, "%27")));   
//        window.location.assign('ShareMain.php?sid=' + sid + "&id=" + idobj + '&name=' + encodeURIComponent(name.replace(/'/g, "%27")) + '&identify=' + type + "&cid=" + cid);   
...
function ShareClickFunction() {
    $('.share').click(function ()
    {
        ...
        /* 2012.11.23 把上一頁按鈕失能. ( 回到 History 第一頁 ) */
        alert(location.href);
        window.location.assign("GetAllSHID.php?sid=" + sid + "&url=" + encodeURIComponent(location.href.replace(/'/g, "%27")));   
...
function CallGetAjax(url, data,datatype) {
    var request = $.ajax({
        type: "Get",
        data: data,
        url: url,

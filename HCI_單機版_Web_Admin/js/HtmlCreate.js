...
//            rtnhtml += '</div>';
        ...
        createobj.CreatevSwitchHtml = function(id, ip, mask,gateway,devs,freeDevs,addFreeeDevs,dhcp) { 
            ...
            var rtnhtml = '<fieldset class="vSwitch_fieldset_external">';
             rtnhtml += '<legend>' + LanguageStr_Network.IP_vSwitch + ' ' + (id+1) + '</legend>'; /* 2017.10.03 william 名詞修改 vSwitch -> 虛擬交換器*/
        ...
        createobj.CreateEXIPListHtmlByTable = function(ajaxData) {
            var rtnhtml = '';
            var no;
            RestoreEXIPList = []; // 要清空array，不然會出錯
            $.each( ajaxData, function(index, element){   
                no = index + 1;   
                var PortTo = (element['ExternalPort'] + element['ExternalPortCount'])-1;
                tmpRestoreIPs = new EXIPListClass(element['ExternalIP'], element['ExternalPort'], element['ExternalPortCount']);
                RestoreEXIPList[element['ExternalIP']] = tmpRestoreIPs;
                                

                if(index === ajaxData.length - 1)
                    rtnhtml += '<div id="EXIPRow' + element['ExternalIP'] + '" class = "Div_VMRow LastVMRow">';
                else
                    rtnhtml += '<div id="EXIPRow' + element['ExternalIP'] + '" class = "Div_VMRow">';
                rtnhtml += '<H4 class = "VMList_ItemNo_Row AbsoluteLayout RowField" >' + no  + '</H4>';
                rtnhtml += '<H4 class = "EXIPList_IP_Row AbsoluteLayout RowField" title="' + element['ExternalIP'] + '">' +  element['ExternalIP'] + '</H4>';
                rtnhtml += '<H4 class = "EXIPList_PortFrom_Row AbsoluteLayout RowField" title="' + element['ExternalPort'] + '">' + element['ExternalPort'] +  '</H4>';
                rtnhtml += '<H4 class = "EXIPList_PortTo_Row AbsoluteLayout RowField" >' + PortTo +  '</H4>';  
                rtnhtml += '<H4 class = "EXIPList_PortCount_Row AbsoluteLayout RowField" >' + element['ExternalPortCount'] + '</H4>';                        
                rtnhtml += '<div class = "EXIPList_Action_Row AbsoluteLayout RowField" >'; 
                rtnhtml += '<a id="' + element['ExternalIP'] + '" class="MenuButton List_Row_Btn EXIPActionBtn" style="left: 85px;">' + LanguageStr_Raid.RAID_Delete + '</a>';
                rtnhtml += '</div>';                        
                rtnhtml += '</div> ';       
            });         
            EXIPListCount = no;
        ...
        createobj.CreateVMListHtmlByTable = function(){            
            ...
            $.each( VMListTable, function(index, element){    
                ...
                HtmlStr += '<H4 class = "VMList_Username_Row AbsoluteLayout RowField" title="' + element['username'] + '">' + element['username'] +  '</H4>';   
        ...
        createobj.CreateSeedListHtmlByTable = function(){            
            ...
            $.each( SeedListTable, function(index, element){   
               ...
               // 2016.12.19 william新增
                ...
                HtmlStr += '<H4 class = "SeedList_Status_Row AbsoluteLayout RowField" >' + createobj.GetVMStatus(element['state']) +  '</H4>';                
                ...
                HtmlStr += '<div id="VMActionRow' + element['name'] +  '" class = "SeedList_Action_Row AbsoluteLayout RowField" >';                                 
                HtmlStr += createobj.CreateSeedDropDownHtml(element['name'],element['state']);      
                HtmlStr += '</div>';                        
                HtmlStr += '</div> ';       
...
//                var type = element["type"] === 0 ? "Original" : "Borned";
                if(index === BornedListTable.length - 1)
                    ...
                    HtmlStr += '<div id="VMRow' + element['name'] + '" class = "Div_VMLognRow" data-status="' + element['state'] + '">';
                HtmlStr += '<H4 class = "VMList_ItemNo_Row AbsoluteLayout RowField" >' + no  + '</H4>';                           
                HtmlStr += '<H4 class = "BornedList_Username_Row AbsoluteLayout RowField" title="' + element['username'] + '">' +  element['username'] + '</H4>';                           
...
//                HtmlStr += '<H4 class = "VMList_MemUtil_Row AbsoluteLayout RowField" >' + element['mem_util'] + '</H4>';        
        ...
        createobj.CreateSeedDropDownHtml = function(name,state){
            var drop_down_content = '';                       
            if(state === 91)
                drop_down_content = '#dropdown-VM-Operation5-1';           
            else
        ...
        createobj.CreateVMListTable = function(JsonData){
            ...
            $.each( JsonData['Org'], function(index, element){                 
                ...
                tmp_vm_class = new VMClass(element['Username'],element['VDName'],0,0,element['State'],0,'',element['Desc'],0,element['CreateTime'],element["Online"],createobj.GetVMStatus(element['State'],element['Online']),element['Suspend'],memory.toFixed(2),element['VDID'],vd_usage_size);                   
                VMListTable.push(tmp_vm_class);
                VMListName[element['VDName']] = tmp_vm_class;   
                VMListUserName[element['VDName']] = tmp_vm_class;                  
            });       
            $.each( JsonData['Seed'], function(index, element){                  
                ...
                SeedNameArray.push(element['VDName'] + ":" + element['VDID']);
            });  
            $.each( JsonData['User'], function(index, element){  
                ...
                vd_usage_size = element['AllSize']; // 桌面設定-新增VD檔案大小實作 2017.05.04 william           
                tmp_vm_class = new VMClass(element['Username'],element['VDName'],0,0,element['State'],0,seedname,element['Desc'],0,element['CreateTime'],element["Online"],createobj.GetVMStatus(element['State'],element['Online']),element['Suspend'],memory.toFixed(2),element['VDID'],vd_usage_size);                    
                BornedListTable.push(tmp_vm_class);
                BornedListName[element['VDName']] = tmp_vm_class;
                BornedListUserName[element['VDName']] = tmp_vm_class;
        ...
        function CreateTaskTypeText(input_type){
            ...
            switch(input_type){
                ...
                case 13:
                    ...
                    break;
                case 14:
                    type = LanguageStr_VM.VM_Task_Overwrite_Seed;
                    break;
                case 15:
                    type = LanguageStr_VM.VM_Task_Reborn_User;
                    break;
                case 16:
                    type = LanguageStr_VM.VM_Task_Recovery_User;
                    break;
                case 17: // 2017.11.22 william 雲桌面轉移至雲主機
                    type = LanguageStr_VM.VM_Task_Clone_to_Org;
                    break;          
                case 18: 
                    type = LanguageStr_VM.Expand_Disk;
                    break;  
                case 19: 
                    type = LanguageStr_VM.Move_Disk;
                    break;  
                case 20: 
                    type = LanguageStr_VM.Move_User_Profile;
                    break;                    
                case 22:
                    type = LanguageStr_VM.VM_Task_Renew_User;
                    break;                                                                    
                default : 
                    type = input_type;
            };
...
//                    return 'Running' + online_desc;
                ...
                case 6:
                    ...
                    break;
                case 7:
                    return '<label style="color:black">Migrating</label>';                    
                    break;
                case 10:
                    return '<label style="color:black">Preparing Boot</label>';                    
                    break;
                case 90:
                    return '<label style="color:black">Latest</label>';                    
                    break;
                case 91:
                    return '<label style="color:black">Renewable</label>';                    
                    break;
                default:
                    return Status;
            }
        ...
        createobj.CreateVMNICListHtml = function(JsonData,isModify){
            ...
            $.each(JsonData['NICs'], function(index, element){
                ...
                HtmlStr += '<label style="width:130px;display:inline-block">Virtual Switch ' + element['vswitch']  + '</label>';
                if(isModify)
        ...
        createobj.CreateVMDiskListHtml = function(JsonData,isModify){
            var HtmlStr = "";         
            var Capacity = "";
            var Type = ""; // 新增磁碟類型  2017.09.04 william
            var Status = "";       // 2018.01.08 william 新增磁碟擴充功能
            IDE_type_count = 0;    // 2018.01.08 william 新增磁碟擴充功能
            SCSI_type_count = 0;   // 2018.01.08 william 新增磁碟擴充功能
            VirtIO_type_count = 0; // 2018.01.08 william 新增磁碟擴充功能            
            HtmlStr += '<div style="height:300px;overflow:auto">';            
            $.each(JsonData['Disks'], function(index, element){
                HtmlStr += '<div style="height:40px;line-height: 40px;">';
                if(index === 0)
                    HtmlStr += '<label class="LabelHead" style="width:120px">' + LanguageStr_VM.VM_Boot_Disk + '</label>';
                else
                    HtmlStr += '<label class="LabelHead" style="width:120px">' + LanguageStr_VM.VM_Data_Disk + '</label>';
                Capacity = element['Size'];                
                Type = element['DiskType']; // 新增磁碟類型  2017.09.04 william    
                Status = element['State']; // 2018.01.08 william 新增磁碟擴充功能           
                HtmlStr += '<label style="width:100px;display:inline-block">' + 'RAID ID : ' + element['RAIDID']  + '</label>'; // 新增磁碟類型  2017.09.04 william
                HtmlStr += '<label style="width:80px;display:inline-block">' + VMDiskTypeStatus(Type)  + '</label>'; // 新增磁碟類型  2017.09.04 william
                HtmlStr += '<label style="width:80px;display:inline-block">' + add_comma_of_number(add_two_fixed_float(Capacity))  + ' GB</label>';
                HtmlStr += '<label style="width:80px;display:inline-block">' + changeVMDiskStatus(Status)  + '</label>';
                // 2018.01.08 william 新增磁碟擴充功能 DiskType :  0:ide 1:scsi 2:virtio             
                switch (Type) {
                    case 0:
                        IDE_type_count++;
                        break;
                ...
                if(isModify && Status === "0") {
                    HtmlStr += '<a id="' + element['DiskID'] + ':' +  Type + ':' + Capacity + '" href="#" class="btn-vm-expend-disk" style="font-size: smaller">' + LanguageStr_VM.VM_Expend_Disk_Txt + '</a>';
                    HtmlStr += '<a id="' + element['DiskID'] + ':' +  element['RAIDID'] + '" href="#" class="btn-vm-move-disk" style="font-size: smaller">' + LanguageStr_VM.Move + '</a>';
                }                  
                if(index !== 0 && isModify){
                    if(element['State'] == 0)                    
        ...
        function VMDiskTypeStatus(type){ // 新增磁碟類型  2017.09.04 william
            switch(type){
                case 0 :
                    return 'IDE';
                case 1 :
        ...
        createobj.CreateImportVDsHtml = function(JsonData){
            ...
            $.each( JsonData, function(index, element){               
                ...
                HtmlStr += '<td style="text-align:center"><input id="import' + element + '" style="width:200px;height:25px" type="text" value="' + element + '"></td>';
                HtmlStr += '<td>';
                HtmlStr += '<select id="combo_raidid_Import' + element + '" class="get_import_raidid" ></select>';                       
                HtmlStr += '</td>';
                HtmlStr += '<td style="text-align:center;line-height: 3;"><a id="' + element + '" href="#" class="btn-vd-import" style="font-size: smaller">'+LanguageStr_VM.VM_Str_Import+'</a></td>';
                HtmlStr += '</tr>';
                i++;
            });             
            return HtmlStr;            
...
//            var HtmlStr = '';            
        ...
        createobj.CreateVMSnapshotTaskHtmlBTable = function(){
            var HtmlStr = '';   
            var no = 0;       

...
class ScheduleAction extends BaseAPI
{
    ...
    function listSchedule($input,&$output){
    	if(!$this->connectDB()){
       	...
       	$outputSchedules = null;        
       	$this->sql_list_schedule_and_jobs_by_idDomain($input['DomainID'],$outputSchedules);
        $i=0;
        $output = array();
        foreach ($outputSchedules as $schedule) {
            if(count($schedule['Jobs']) == 0 ){
                $this->sql_delete_schedule_by_id($schedule['ScheduleID']);                
            }
            else{
                $output[]=$schedule;
            }            

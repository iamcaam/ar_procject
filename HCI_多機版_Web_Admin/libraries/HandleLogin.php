...
class Login  extends BaseAPI{
    ...
    // public function Sec_Session_Start($session_path="/storage/ceph/LocalDB/admin/session") {     
        // var_dump($session_path);  
...
//        echo 1;
    ...
    function Login_By_Data($sInputUserName, $sInputPassword,$session_path="/LocalDB/admin/session",$vmsIP=NULL,$vmsMask=NULL) 
    // function Login_By_Data($sInputUserName, $sInputPassword,$session_path="/storage/ceph/LocalDB/admin/session") 
    {        
        // if(!$this->connectDB()){
        //     return CPAPIStatus::DBConnectFail;
        // }        
        $this->getSANPWDSlat($UserPassword,$UserSalt);         
        ...
        if ($UserPassword === $sPassword) {            
            ...
            // var_dump($vmsIP);
            if(isset($vmsIP)){
                $_SESSION['VMSIP'] = $vmsIP;
                $_SESSION['VMSMask'] = $vmsMask;
            }
            // var_dump($_SESSION);
            session_write_close();
            exec("rm -f /mnt/tmpfs/LocalDB/manager/session/*",$output,$rtn);           
            for($i=0;$i<3;$i++)
            {            
                exec("cp -f $session_path/* /mnt/tmpfs/LocalDB/manager/session/",$output,$rtn);
                if($rtn!=0)
                    continue;
            }
            // Login successful. 
            ...
            // LogAction::createLog(array('Type'=>4,'Level'=>1,'Code'=>'04100F01','Riser'=>'admin','Message'=>"Admin Login AcroVMS Administrator Success"),$last_id);            
            return APIStatus::LoginSuccess;
        } else {                    
            // LogAction::createLog(array('Type'=>4,'Level'=>3,'Code'=>'04301001','Riser'=>'admin','Message'=>"Admin Login AcroVMS Administrator Fail"),$last_id);            
            return APIStatus::LoginFail;
        }           
    }
    
    function getSANPWDSlat(&$pwd,&$slat)
    {
         if(!file_exists("/artgluster/LocalDB/manager/session"))
            $pwdRootPath = "/LocalDB/manager/";
        else
            $pwdRootPath = "/artgluster/LocalDB/manager/";        
        $pwd = trim(file_get_contents($pwdRootPath.'pwd'));   
        $slat = trim(file_get_contents($pwdRootPath.'slat'));
    }

    function Login_By_Data_Web($sInputUserName, $sInputPassword, $token, $session_path="/LocalDB/admin/session") {    
    // function Login_By_Data_Web($sInputUserName, $sInputPassword, $token, $session_path="/storage/ceph/LocalDB/admin/session") {              
        // var_dump($session_path);
        $UserPassword = trim(file_get_contents($this->pwd_path));   
        $UserSalt = trim(file_get_contents($this->slat_path));
        // hash the password with the unique salt.             
        ...
        if ($UserPassword === $sPassword) {                
            ...
            // Get the user-agent string of the user.            
            $sUserBrowser = $_SERVER['HTTP_USER_AGENT'];                        
            fwrite($fp, $sUserBrowser.PHP_EOL);          
            // XSS protection as we might print this value
            $sUsername = preg_replace("/[^a-zA-Z0-9_\-]+/", "", $sInputUserName);
            shell_exec("rm -f $session_path/*");              
            $this->Sec_Session_Start($session_path);
            $_SESSION['Username'] = $sUsername;
            $_SESSION['test'] = $sUserBrowser;
            $_SESSION['LoginString'] = hash('sha512', $sPassword . $sUserBrowser);
            $_SESSION['UserBrowser'] = $sUserBrowser;
            if(isset($token))
    ...
    public function Change_Password($sOldPWD,$sNewPWD,$pwdPath='/LocalDB/admin/pwd',$slatPath='/LocalDB/admin/slat'){                        
        $UserPassword = trim(file_get_contents($pwdPath));   
        // var_dump($UserPassword);
        $UserSalt = trim(file_get_contents($slatPath));
        // var_dump($UserSalt);        
        ...
        if (file_put_contents($pwdPath, $sNewPWD)) {                 
    ...
    public function Login_Check($input) {    
     ...
     public function sanLoginCheck($input) {    
        // var_dump($input);
        // var_dump($_SESSION);         
        if (isset($_SESSION['Username'], $_SESSION['LoginString'])) {                 
            $sLoginString = $_SESSION['LoginString'];            
            ...
            session_write_close();                                                
            // $UserPassword = trim(file_get_contents($this->pwd_path));
            $this->getSANPWDSlat($UserPassword,$slat);            
            $sLoginCheck = hash('sha512', $UserPassword . $sUserBrowser);           
            if(isset($input['token'])){
                if($input['token'] != $token)
                    return APIStatus::LoginFail;
    ...
    public function loginCheckServer()
    {
        $this->connectDB();        
        if($this->dbh == null){
            return CPAPIStatus::DBConnectFail;
    ...
    function sqlListServers(&$outputServers)
    {
        $sqlListServers = "SELECT * FROM tbvdserverset;";
        try        
        {  
            $sth = $this->dbh->prepare($sqlListServers);           
            if($sth->execute())
            {                             
                $outputServers = array();                 
                while( $row = $sth->fetch() ) 
                {                         
                    $outputServers[]=array('ID'=>$row['idVDServer'],'Name'=>$row['nameNode'],'Address'=>$row['address']);
                }                                 
            }               
        }
        catch (Exception $e){                
